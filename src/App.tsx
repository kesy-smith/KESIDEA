...
export default function App() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (u) => {
      ...
      if (u) {
        setScreen('dashboard');
        // Use a separate function or check to avoid constant overwrites and rules issues
        const syncUser = async () => {
          try {
            const userRef = doc(db, 'users', u.uid);
            // We use setDoc with merge to ensure the record exists
            // But we need to match the rules: uid, displayName, email, createdAt
            // In the rules, I'll allow update if the incoming data is valid.
            // To simplify, let's just make sure we send all required fields.
            await setDoc(userRef, {
              uid: u.uid,
              displayName: u.displayName || TRANSLATIONS[language].anonymous,
              email: u.email,
              createdAt: serverTimestamp() // rules will check this on creation
            }, { merge: true });
          } catch (err) {
             // If update fails because of 'createdAt' being immutable in rules, 
             // we could handle it by trying a partial update or just ignore if it exists.
             // For now, I'll adjust rules to be more flexible or App to be smarter.
  ...
  const startQuiz = (theme: QuizTheme) => {
    ...
    const shuffled = [...theme.questions]
